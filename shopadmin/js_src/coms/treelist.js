...
var GoodsCatNavTreeList=new Class({
       Extends:TreeList,
       createNode:function(data){
             var options=this.options;
             options.navURL=options.navURL||'#';
            var nc=options.nodeClass;
          var node_handle=new Element('span',{'class':nc.handle})
                          .set({'pid':data['PID'],
                                'nid':data['NID'],
          ...
          var node_icon=new Element('span',{'class':nc.icon}).setHTML('&nbsp;');*/
          var url= options.navURL.substitute({
              cname:data['CNAME'],
              cid:data['NID'],
              cnamelth:data['CNAME'].getLength()
          });
		  
		  url=url.split('filter=');     
		  url=url[0]+'filter='+encodeURIComponent(url[1]);
		  
          var node_name=  new Element('a',{'class':nc.name,'href':url}).setText(data['CNAME']);
          
          var node=new Element('span',{'class':nc.clazz})
               .adopt([node_handle,node_name]);
               
          if(data['NID']==0){
             node_name.setStyle('color','red');
          }         
