...
var getGcptHTML=function(){
   ...
   var nk={WM:['#000','#444','#666','#999','#ccc','#eee','#f3f3f3','#fff'],
   ...
   var gps_u='<div class="goog-palette">'
                     +'<table cellspacing="0" cellpadding="0"  class="goog-palette-table">'
   ...
   $H(nk).each(function(colors,part){
        ...
        colors.each(function(color,index){
           ...
           gp+='<td class="goog-palette-cell">'
               +'<div title="'+rgb+'" style="background-color: '+rgb+';" class="goog-palette-colorswatch"/></td>';
           if((index+1)%8==0){
...
GoogColorPicker=new Class({
			...
			position: function(event){
				...
				var gcp_panel = {x: this.gcp_panel.offsetWidth, y: this.gcp_panel.offsetHeight};
            ...
            hidePanel:function(){
               ...
               if(this.gcp_panel)
			   this.gcp_panel.setStyle('visibility','hidden');
			},
			showPanel:function(event){
               var that=this;
               
               var el_uid=this.el.uid||Native.UID++;
               if(!$('gcp_panel'+el_uid)){
                   var gcp_panel=this.gcp_panel=$('gcp_panel'+el_uid)||
                                 new Element('div',{'id':'gcp_panel'+el_uid,'class':'goog-palette-panel'}).setHTML(getGcptHTML())
                                 .inject($('tabernacle'));
